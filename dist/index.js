...
const ci_1 = __nccwpck_require__(239);
...
class GithubHelper {
    ...
    async listAllBranches(owner, repo, page = 1) {
        const branches = await node_fetch_1.default(GithubHelper.branchesEndpoint(owner, repo, page), { headers: { 'Authorization': `token ${this.githubToken}` } })
            .then((r) => r.json());
        if (branches.length) {
            return [
                ...branches,
...
GithubHelper.branchesEndpoint = (owner, repo, page = 1, perPage = 100) => `https://api.github.com/repos/${owner}/${repo}/branches?page=${page}&per_page=${perPage}`;
...
const branch_utils_1 = __nccwpck_require__(10);
...
async function run() {
    ...
    const githubHelper = new github_helper_1.GithubHelper(githubToken);
